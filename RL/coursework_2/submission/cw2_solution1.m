...
for episode = 1:100
    ...
    for i = 1:episodeLength
        ...
        increment = agentRewardSignal + lambda*compV(next_state, weights)...
            - compV(stateFeatures, weights);
        
        % Update the weights
        for col = 1:5
            for row = 1:4
                weights(row, col) = weights(row,col)...
                    +(alpha/counter)*increment*stateFeatures(row, col);
            end
        end
        
        % Print V(s) weights on screen
        weights
        ...
        % Increment the counter to decrease the learning rate
        counter = counter+1;
    end
...
function [V] = compV(features, weights)
    V= sum ( sum( weights .* features) );
end